...
process minimap2_alignment {
    ...
    input:
        path reference
        tuple val(meta), path(reads), path(reads_idx), path(stats)
        tuple val(align_ext), val(index_ext) // either [bam, bai] or [cram, crai]

    output:
        tuple val(meta), path("${meta.alias}.${align_ext}"), path("${meta.alias}.${align_ext}.${index_ext}"), path(stats), emit: alignment
    script:
    """
    samtools bam2fq -@ 2 -T 1 ${reads} \
        | minimap2 -y -t 2 -ax map-ont --cap-kalloc 100m --cap-sw-mem 50m \
            ${reference} - \
        | samtools sort -@ 2 \
...
process check_for_alignment {
    ...
    input:
        ...
        tuple val(meta), path(xam), path(xam_idx), path(stats)
    output:
        tuple env(to_align), val(meta), path(xam), path(xam_idx), path(stats)
    script:
        """
        to_align=0
...
workflow ingress {
    ...
    main:
        ...
        checked_bam = check_for_alignment(
            ...
            map{ to_align, meta, xam, xai, stats ->
                [meta + [to_align: to_align != '0'], xam, xai, stats]
            }
